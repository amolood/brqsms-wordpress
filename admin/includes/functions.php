...
function digits_update_data($gs)
{
    ...
    $data = array(
            ...
            'digits_captcha_provider',
            'digits_recaptcha_site_key',
            'digits_recaptcha_secret_key',
            'digits_recaptcha_type',
            'digits_turnstile_site_key',
            'digits_turnstile_secret_key',
            'digits_hcaptcha_site_key',
            'digits_hcaptcha_secret_key',
            'digits_hcaptcha_type',
            ...
            'digits_add_countrycode_in_billing_phone',
            'digits_email_verification_method',
            'digits_enhance_wc_checkout_phonenumber',
            'dig_forgot_pass_otp_method'
    );
    foreach ($data as $key) {
        if (isset($_POST[$key])) {
    ...
    if (isset($_POST['dig_login_default_tab'])) {
        $dig_login_default_tab = sanitize_text_field($_POST['dig_login_default_tab']);
        update_option('dig_login_default_tab', $dig_login_default_tab);
    } else {
        update_option('dig_login_default_tab', 'phone');
    }
...
{
    ?>
    <div class="dig_addons_coming_soon"><?php _e('This is the open-source BrqSMS build.', 'digits'); ?></div>
    <?php
    return;

    $url = "https://dash.brqsms.com/wp-admin/admin-ajax.php?action=digits_addons&get=products&type=json&data=addons&version=" . digits_version() . "&purchasecode=" . dig_get_option('dig_purchasecode');

    if ($active_tab == 'addons') {
        $digits_addon_cache_buster = set_transient('digits_addons_cache_buster', time(), DAY_IN_SECONDS);
    }

    $digits_addon_cache_buster = get_transient('digits_addons_cache_buster');
    if (!empty($digits_addon_cache_buster)) {
        $url .= "&cache_buster=" . time();
    }
    ...
    <div class="digits_activation_wrapper">
        ...
        <table class="dig_prchcde" <?php if (!empty($code)) {
            ...
            <tr>
                <td>
                    <?php
                    $manage_link = 'https://dash.brqsms.com';
                    if (!empty($code)) {

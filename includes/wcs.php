        ...
        if ($dig_hid_update_notice < 1) {
            ...
            if (($current_time - $time) > 2592000) {
                ...
                <div class="notice notice-error dig-new-activation-notice is-dismissible">
                    <p>
                        <?php _e('<b>BrqSMS:</b> Please register BrqSMS with license key to enable automatic updates. Click <a href="' . $request_link . '">here</a>.<br />'); ?>
                    </p>
    ...
    /*
        ...
        if (class_exists('WooCommerce')) {
            if ($passaccep == 0 && get_option('woocommerce_registration_generate_password') === 'no') {
                ...
                $message = __('<b>BrqSMS:</b> Please enable <b>Automatically generate customer password</b> option in your WooCommerce settings (WooCommerce --> Settings --> Accounts) for disabling password.', 'digits');
...
{
    add_meta_box('digits_endpoints_nav_link', __('BrqSMS Menu Items', 'digits'), 'dig_nav_menu_links', 'nav-menus', 'side', 'low');
}
...
function digits_filter_username($username, $type = '')
{
    $username = preg_replace('/\s*/', '', $username);
    $username = digits_strtolower($username);

    if ($type == 'name' && strlen($username) > 49) {

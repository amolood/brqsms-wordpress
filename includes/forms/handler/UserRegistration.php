...
class UserRegistration
{
    ...
    public function process()
    {
        ...
        $digits_email_verification_method = get_option('digits_email_verification_method', 'otp');
        $email = '';
        $mobile = '';
        $countrycode = '';
        ...
        if (!empty($action_type)) {
            if ($mobileaccp == 1 || $emailaccep == 1) {
                ...
                if ($action_type == 'email') {
                    $base_countrycode_key = $countrycode_key;
                    $base_phone_key = $phone_key;
                    $countrycode_key = $optional_key . $countrycode_key;
                    $phone_key = $optional_key . $phone_key;
                    if (empty($this->get($phone_key, false))) {
                        $countrycode_key = $base_countrycode_key;
                        $phone_key = $base_phone_key;
                    }
                } else {
                    $base_email_key = $email_key;
                    $email_key = $optional_key . $email_key;
                    if (empty($this->get($email_key, false))) {
                        $email_key = $base_email_key;
                    }
                }
        ...
        if ($mobileaccp == 2) {
            ...
            if (!digits_validate_phone($phone)) {
                $validation_error->add("Mobile", __("Please enter a valid Mobile Number!", "digits"));
            }
        ...
        $email_otp = false;
        $need_to_delete_email_otp = false;
        if (!empty($email)) {
            if (self::isEmailVerificationRequired()) {
                if ($digits_email_verification_method == 'otp') {
                    $otp_type = 'email_otp';
                    if ($sub_action == $otp_type) {
                        $data = array();
                        $request = $this->send_otp($sub_action, false, false, $email);
                        $data = array_merge($data, $request);
                        wp_send_json_success($data);
                    }
                    $email_otp = $this->get($otp_type, false);

                    if (!empty($this->data['digits_email_otp_field']) && empty($email_otp)) {
                        $validation_error->add("invalidOTP", __("Please enter a valid OTP!", "digits"));
                    }
                    $this->check_errors($validation_error);
                    if (empty($email_otp) && empty($this->get('otp_step_2', false))) {
                        wp_send_json_success($this->show_html_step('otp_section', ['method' => $otp_type]));
                    }
                    try {
                        $verify_email = $this->isValidOTP($otp_type, false, false, $email, $email_otp, false);
                        if (!$verify_email) {
                            $validation_error->add("invalidOTP", __("Please enter a valid OTP!", "digits"));
                        } else if ($verify_email instanceof WP_Error) {
                            $validation_error->merge_from($verify_email);
                        } else {
                            $need_to_delete_email_otp = true;
                            $email_verified = true;
                        }
                    } catch (\Exception $e) {
                        $validation_error->add("invalidOTP", $e->getMessage());
                    }
                }
        ...
        if (!empty($mobile)) {
            if ($skip_otp_verification == 0) {
                ...
                if (dig_isWhatsAppEnabled() && dig_isSMSEnabled()
                ...
                if ($sub_action != 'email_otp' && $this->is_otp_action($sub_action)) {
                    $data = array();
                    $request = $this->send_otp($sub_action, $countrycode, $mobile, false);
                    $data = array_merge($data, $request);
                    wp_send_json_success($data);
                }
                ...
                try {
                    $verify_phone = $this->isValidOTP($otp_type, $countrycode, $mobile, false, $otp, true);
                    if (!$verify_phone) {
                        $validation_error->add("invalidOTP", __("Please enter a valid OTP!", "digits"));
                    } else if ($verify_phone instanceof WP_Error) {
                        $validation_error->merge_from($verify_phone);
                    } else {
                        $phone_verified = true;
                    }
        ...
        if ($need_to_delete_email_otp) {
            $verify_email = $this->isValidOTP('email_otp', false, false, $email, $email_otp, true);
        }
    ...
    public function check_errors($validation_error)
    {
        if (!empty($validation_error->get_error_codes())) {
            $errors = array_map(function ($msg) {
                return str_replace('login', 'verification', $msg);
            }, $validation_error->get_error_messages());
    ...
    {
        $response = array();
        $response['html'] = $this->render_step_html($action, $data);
        return $response;
    }
    ...
    public function send_otp($action, $country_code, $mobile, $email)
    {
        $details = array();
        if (!empty($mobile)) {
    ...
    public function isValidOTP($otp_type, $country_code, $phone, $email, $otp, $delete_otp)
    {
        $verify_func = 'verify_phone_otp';
        if ($otp_type == 'whatsapp_otp') {
            $verify_func = 'verify_whatsapp_otp';
        } else if ($otp_type == 'email_otp') {
            return Handler::instance()->verify_email_otp($email, $otp, $delete_otp);
        }
    ...
    public function render_otp_mode_selector()
    {
        ...
        $methods = array_diff($methods, array('email_otp'));
        Processor::instance()->otp_tab(0, $methods, 1, 'register');
        $title = __('Verify', 'digits');
    ...
    public function render_tab_optional_field($action, $data)
    {
        ...
        <div data-field-type="<?php echo $type; ?>"
            ...
            if ($type == 'phone') {
                ...
                digits_ui_reg_phone_field('optional_', $userCountryCode, $userCountry, -1, false);
            } else if ($type == 'email') {
                digits_ui_reg_email_field('optional_email', -1, false);
            }
            ?>
    ...
    public function render_tab_otp_field($action, $data)
    {
        ...
        $step_no = 1;
        $title = __('Verify Phone', 'digits');
        $otp_field = 'digits_otp_field';
        if (is_array($data)) {
            if (isset($data['method'])) {
                $method = $data['method'];
                $title = __('Verify Email', 'digits');
                $step_no = 2;
                $otp_field = 'digits_email_otp_field';
            }
        }

        Processor::instance()->render_otp_box(0, $method, $step_no, 'register');

        ?>
        <input type="hidden"
               class="digits-form_otp_selector auto-click dig_process_data"
               data-disable_update="1"
               data-type="<?php echo esc_attr($method); ?>"/>
        <input type="hidden" name="<?php echo esc_attr($otp_field); ?>" value="1"/>
        <?php
        echo '<span class="main-section-title digits_display_none">' . esc_attr($title) . '</span>';

...
final class Handler
{
    ...
    public function __construct()
    {
        ...
        add_filter('authenticate', [$this, 'login_error_message'], 100, 3);
    ...
    public function login_error_message($user, $username, $password)
    {
        if ($user instanceof WP_Error) {
            if (strpos($user->get_error_code(), 'incorrect') !== false) {
                return new WP_Error(
                    'incorrect_password',
                    __('<strong>Oops!</strong> The password you entered is not correct.', 'digits')
                );
            }
        }
        return $user;
    ...
    public function check_remote_approve_status($method)
    {
        ...
        if ($method == 'whatsapp_otp') {
            $identifier_id = $this->get('wa_otp_token_key', false);
                ...
                <div class="digits_secure_fingerprint_container">
                    <div class="digits_secure_phone_qr_wrap">
                        <div class="digits_secure_phone_qr_container">
                            ...
                            <input type="hidden" class="wa_otp_token_verify_status" name="wa_otp_token_verify_status" />
                            <input type="hidden" class="wa_otp_token_key" value="<?php echo esc_attr($wa_data['key']); ?>"
                                name="wa_otp_token_key" />
                            <input type="hidden" class="whatsapp_otp_hidden_field" value="" name="whatsapp_otp" />
                        </div>
                    </div>
                </div>
    ...
    public function authenticate_2fa_auth_app_code($user, $otp, $step_no)
    {
        ...
        $details = ['user_id' => $user_id, 'login_type' => '2fa_app'];


        $is_blocked = \DigitsBruteForceProtection::is_user_blocked($details);
        if ($is_blocked instanceof WP_Error) {
            return $is_blocked;
        }
        ...
        \DigitsBruteForceProtection::add_invalid_otp_attempt($details);
    ...
    private function verify_otp($email, $country_code, $phone, $otp, $delete_otp, $route, $validity = 1200)
    {
        ...
        WHERE ' . $column . ' = %s AND otp=%s AND action_type=%s ORDER BY time DESC LIMIT 1',
            ...
            $details['phone'] = $country_code . $phone;
        ...
        if ($verify_row) {
            ...
            $details['login_type'] = $route == 'user' ? 'user_otp' : $route;
    ...
    public function forgotpass()
    {
        ...
        $forgot_pass_method = digits_forgot_password_otp_method();
        if ($forgot_pass_method == 'email') {
            $action = 'email_otp';
            $otp_key = 'email_otp';
        } else if ($forgot_pass_method == 'sms') {
            $action = 'sms_otp';
            $otp_key = 'sms_otp';
        }

        if ($otp_key == 'email_otp' && empty($email)) {
            $email = $user->user_email;
            if (empty($email)) {
                throw new Exception(__('There is no email address linked to the account!', 'digits'));
            }
        } else if ($otp_key == 'sms_otp' && empty($phone)) {
            $phone_obj = digits_get_mobile_country_code($user->ID);
            if (empty($phone_obj)) {

...
{
    // Prevent caching of forms with nonces
            ...
            <td style="position: relative;">
                ...
                <div class="digits_edit_user_phone_div">
                    ...
                    <input type="text" autocomplete="off"
                        ...
                        data-dig-mob="1" name="dig_user_mobile" id="username" nan="1" data-type="2"
        ...
        if (current_user_can('edit_users')) {
            ...
            if (!empty($digits_data) && is_array($digits_data)) {
                ...
                foreach ($digits_data as $data_meta_key => $data) {
                    if (in_array($data_meta_key, $defined_fields))
                        continue;
                    ...
                    <tr>
                        ...
                        <td style="position: relative;">
                            ...
                            ?>
                            <input type="hidden" name="digits_undefined_fields[]" value="<?php esc_html_e($data_meta_key); ?>" />
                            <input type="text" name="digits_field_<?php echo esc_html($data_meta_key) ?>" class="regular-text"
    ...
    if (is_admin()) {
        ...
        $verified_html = '<span class="digits_admin_verify_status_ic digits_admin_verified" title="' . esc_attr__('%s Verified by BrqSMS', 'digits') . '"></span>';
        $not_verified_html = '<span class="digits_admin_verify_status_ic digits_admin_not_verified" title="' . esc_attr__('%s Not Verified by BrqSMS', 'digits') . '"></span>';
        ...
        <style>
            ...
            .wp-admin #profile-page .digcon .mobile_field {
                padding-left: 75px;
            }
            .digits_edit_user_phone_div .digcon .mobile_number{
                padding-left: 70px;
            }
        </style>
        <?php
...
function validate_info($errors, $update = null, $user = null)
{
    ...
    if ((!isset($_POST['dig_user_mobile']) || !isset($_POST['digt_countrycode'])) && !isset($_POST['mobile_email'])) {

        $errors->add('MobileNo', "<strong>" . __("Error", "digits") . "</strong>: " . __("Invalid Mobile Number!", "digits"));

    } else if ((!isset($_POST['digt_countrycode']) || !isset($_POST['mobile_email']) || !isset($_POST['dig_old_phone'])) && !isset($_POST['dig_user_mobile'])) {
        $errors->add('MobileNo', "<strong>" . __("Error", "digits") . "</strong>: " . __("Invalid Mobile Number!", "digits"));
    }
    ...
    } else if (isset($_POST['mobile_email'])) {
        $countrycode = sanitize_text_field($_POST['digt_countrycode']);
        $phone = sanitize_text_field($_POST['mobile_email']);
...
function digits_override_checkout_fields($fields)
{
    ...
    if ($mobileaccp > 0) {
        ...
        $fields['account']['mobile_email']['required'] = true;
        $fields['account']['mobile_email']['id'] = "username";

        $fields['account']['mobile_email']['priority'] = 1;
        $fields['account']['mobile_email']['class'] = array('form-row-wide');

        if ($mobileaccp == 2) {
            $fields['account']['mobile_email']['placeholder'] = __("Mobile Number", "digits");
            $fields['account']['mobile_email']['label'] = __("Mobile Number", "digits");
        } else {
            $fields['account']['mobile_email']['placeholder'] = __("Email/Mobile Number", "digits");
            $fields['account']['mobile_email']['label'] = __("Email/Mobile Number", "digits");
        }
        if (isset($fields['billing']['billing_email'])) {
...
if (!function_exists('wc_create_new_customer')) {
    ...
    function wc_create_new_customer($email, $username = '', $password = '', $args = array())
    {
        ...
        if (empty($nonce_value) || !wp_verify_nonce($nonce_value, 'woocommerce-process_checkout')) {
            ...
            $is_checkout = true;
            $otp = false;
            $phone_number = $_POST['mobile_email'];
            if (isset($_POST['digit_ac_otp'])) {
...
function validate_digits_wc_billing()
{
    ...
    if (dig_get_checkout_otp_verification() == 0 || is_user_logged_in() || 'yes' !== get_option('woocommerce_enable_signup_and_login_from_checkout') || empty($_POST['mobile_email'])) {
        return;
    }
    ...
    $phone_number = sanitize_mobile_field_dig($_POST['mobile_email']);
...
function digit_woocommerce_after_checkout_billing_form()
{
    ...
    $digits_enhance_wc_checkout_phonenumber = get_option('digits_enhance_wc_checkout_phonenumber', 1);
    if ($digits_enhance_wc_checkout_phonenumber == 1) {
        ?>
...
function dig_update_user_profile($user_id)
{
    ...
    if (!current_user_can('edit_user') && !current_user_can('administrator')) {
        ...
        $countrycode = sanitize_text_field($_POST['digt_countrycode']);
        $p = sanitize_text_field($_POST['mobile_email']);
...
function wooc_extra_login()
{
    if (digits_make_third_party_secure()) {
        ...
        <button onclick="return false"
            class="woocommerce-Button button digits_secure_login-tp digits-form_submit-btn"><?php _e('Continue', 'digits'); ?></button>
        ...
        <div class="loginViaContainer">
            ...
            if ($otpaccep == 1) {
                ...
                <?php if (dig_isWhatsAppEnabled()) { ?>
                    <button onclick="return false" class="woocommerce-Button button dig_wc_mobileLogin dig_wc_mobileWhatsApp"
...
function wooc_add_extra_otp_reg_field()
{
    ...
    <input type="hidden" name="dig_nounce" class="dig_nounce" value="<?php echo wp_create_nonce('dig_form') ?>">
    <p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide" id="reg_billing_otp_container"
        style="display: none;">
...
function wooc_add_extra_reg_field()
{
    ...
    if ($emailaccep > 0 && $mobileaccp > 0) {
        ...
        ?>
        <div id="dig_cs_email" class="dig_wc_mailsecond dig-custom-field" <?php if ($emailaccep > 1 || $mobileaccp > 1)
            echo 'style="display:block;"' ?>>
                <p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
                    <label for="secondmailormobile"><span id="dig_secHolder"><?php echo $emailmob; ?></span><span>
                        <?php echo $reqoropt; ?></span></label>
                <input class="woocommerce-Input woocommerce-Input--text input-text secmailormob" name="secondmailormobile"
                    id="secondmailormobile" type="text" <?php if ($emailaccep == 2)
                        echo "required" ?>>
                </p>
            </div>
        <?php
...
function wc_edit_act()
{
    ...
    <p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
        ...
        <input type="text" class="woocommerce-Input woocommerce-Input--email input-text dig_wc_nw_phone"
            name="account_email" id="username" mob="1" f-mob="1"
            countryCode="<?php echo esc_attr(get_the_author_meta('digt_countrycode', $user->ID)); ?>"
            value="<?php echo esc_attr(get_the_author_meta('digits_phone_no', $user->ID)); ?>">
...
function wc_dig_lost_pass()
{
    ...
    <div class="changePassword" style="display: none;">
        <p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
            <label for="reg_password"><?php _e('New Password', 'digits'); ?> <span class="required">*</span></label>
            <input class="woocommerce-Input woocommerce-Input--text input-text" name="digits_password" id="dig_wc_password"
                type="password">
        </p>
        <p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
...
function register_digits_exporter($exporters)
{
    $exporters['digits-customer-data'] = array(
        'exporter_friendly_name' => __('BrqSMS'),
...
function register_digits_plugin_eraser($erasers)
{
    $erasers['digits-customer-data'] = array(
        'eraser_friendly_name' => __('BrqSMS Data Eraser'),

        ...
        if ($type == -1) {
            ...
            if (is_wp_error(validate_plugin($plugin))) {
                ...
                if (!empty($_REQUEST['wordpress'])) {
                    ...
                    $checkPurchase = dig_doCurl('https://dash.brqsms.com/updates/?action=get_metadata&slug=' . $slug . '&license_key=' . $digpc . '&request_site=' . dig_network_home_url());

                    if (!isset($checkPurchase['download_url'])) {
                        $is_subscription = str_starts_with(strtolower($digpc), "digits");
                        $error_msg = sprintf(__('To use this add-on, please update your subscription by clicking %shere%s', 'digits'), '<a href="https://dash.brqsms.com" target="_blank">', '</a>');
                        if (!$is_subscription) {
                            $error_msg = __('Please purchase addon license from https://dash.brqsms.com', 'digits');
                        }
                        $status['errorMessage'] = $error_msg;
                        wp_send_json_error($status);
                    }
                    $plugin_url = 'https://dash.brqsms.com/updates/?action=download&slug=' . $slug . '&license_key=' . $digpc . '&request_site=' . dig_network_home_url();
                }

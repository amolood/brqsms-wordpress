...
require_once plugin_dir_path(__FILE__) . 'includes/default_emails.php';
...
require_once plugin_dir_path(__FILE__) . 'includes/wp_page.php';
...
require_once plugin_dir_path(__FILE__) . 'includes/brute-protection.php';
...
function digits_update_plugin_meta($plugin_meta, $plugin_file, $plugin_data, $status)
{
    ...
    if ($slug == 'digits') {
        $plugin_meta['2'] = '<a href="https://dash.brqsms.com" target="_blank">' . __('View changelog', 'digits') . '</a>';
    }
...
function getCountry()
{
    ...
    if (empty($countryname)) {
        ...
        if ($force && function_exists('curl_init')) {
            $ch = curl_init();
        ...
        <div id="dig-ucr-container" class="dig-box" style="display: none;">
            <div class="dig-content">
                ...
                <p>
                    ...
                    <input type="text" id="dig-cru-lastname" name="lastname" placeholder="<?php _e('Last Name', 'digits'); ?>"
                        autocomplete="name" style="direction: <?php echo $dir ?>;" />
                    <input type="text" id="username" class="dig-cru-mailormob" name="emailormobilenumber"
                        placeholder="<?php _e('Email/Mobile Number', 'digits'); ?>" autocomplete="off" /><br />
                <div class="cancelccb button"><?php _e('Cancel', 'digits'); ?></div>
                <div class="createcustomer dig_createcustomer button button-primary"><?php _e('Create Customer', 'digits'); ?>
                </div>
                <br />
                </p>
...
function digits_new_ui_modal()
{
    ...
    ?>
        ...
        <div class="digits_login_form digits_ui digits_main_wrapper <?php echo esc_attr($digits_modal_class); ?>"
            id="digits_protected">
                    ...
                    <div id="dig-ucr-container" class="<?php if (is_rtl()) {
                      ...
                      if ($page_type == 2) {
                          ...
                          echo ' dig_pgmdl_1';
                           ...
                           echo 'data-placeholder="yes"'; ?>
                        data-asterisk="<?php echo get_option('dig_show_asterisk', 0); ?>" style="display:none;">
...
function digits_activate()
{
    ...
    if (version_compare(PHP_VERSION, '5.5', '<') && is_admin()) {
        $version_required = sprintf('<div><p>You are currently using outdated version of PHP %1$s. Please update your PHP to newer version, BrqSMS requires PHP v5.5 or higher to work. </p></div>', PHP_VERSION);
        wp_die($version_required);
    }

    if (!extension_loaded('gmp') && !extension_loaded('bcmath')) {
        wp_die(__('<div><p><b>Fatal Error</b>: BrqSMS requires GMP or BCMath to work correctly. </p></div>', 'digits'));
    }

    if (!function_exists('curl_version')) {
        wp_die(__('<div><p><b>Fatal Error</b>: BrqSMS requires curl to work correctly. </p></div>', 'digits'));
    }
...
{
    // Prevent caching of login pages with nonces
    if (!defined('DONOTCACHEPAGE')) {
        define('DONOTCACHEPAGE', true);
    }
                            ...
                            <div class="dig_bdy_container <?php if ($page_type == 2) {
                                ...
                                <div class="dig_lrf_box digits_modal_box dig_ma-box <?php if ($page_type == 2) {
                                    ...
                                    echo 'dig_pgmdl_1';
                                     ...
                                     echo 'data-placeholder="yes"'; ?>
                                    data-asterisk="<?php echo get_option('dig_show_asterisk', 0); ?>">
                                    ...
                                    <div class="dig_clg_bx" style="opacity: 0;">
                                        ...
                                        <?php if ($page_type == 2) { ?>
                                                <div class="dig_login_cancel">
                                                    <a href="<?php echo $current_url; ?>" class="dig_page_cancel_color" <?php if (!empty($backcolor)) {
                                                           echo 'style="color:' . $backcolor . ';"';
    ...
    foreach ($reg_custom_fields as $label => $values) {
        ...
        if ($required == 1 && empty($e_value)) {
            ...
            if ($type == "captcha") {
                ...
                if (
                    isset($_SESSION['dig_captcha' . $ses . '_time']) &&
...
function dig_show_login_captcha($login_page = 1, $bgtype = null, $user_id = 0, $extra_class = '', $placeholder = false, $captcha = 1)
{
    ...
    if ($captcha == 2) {
        ...
        if ($provider === 'turnstile') {
            $site_key = get_option('digits_turnstile_site_key');
            ...
            ?>
                            <div class="<?php echo esc_attr($extra_class); ?>">
                                <div class="cf-turnstile dig-turnstile" isolated="true" data-sitekey="<?php echo esc_attr($site_key); ?>"
        ...
        if ($provider === 'hcaptcha') {
            $site_key = get_option('digits_hcaptcha_site_key');
            ...
            ?>
                            <div class="<?php echo esc_attr($extra_class); ?>">
                                <div class="dig-hcaptcha" isolated="true" data-sitekey="<?php echo esc_attr($site_key); ?>"
                                    data-size="<?php echo esc_attr($size); ?>"></div>
                            </div>
                            <?php
                            // Explicit rendering with onload callback to avoid race conditions.
                            wp_enqueue_script(
                                'digits-hcaptcha',
                                'https://js.hcaptcha.com/1/api.js?render=explicit&onload=digits_hcaptcha_onload&recaptchacompat=off',
                                array('digits-frontend-script'),
                                null,
                                true
                            );
                            return;
        ...
        ?>
                    <div class="<?php echo $extra_class; ?>">
                        <div class="<?php echo $captcha_class; ?>" isolated="true" data-ctype="<?php echo esc_attr($captcha_type); ?>"
                            data-callback="digits_recaptcha_callback" data-render="explicit"
                            id="<?php echo esc_attr(uniqid('captcha_')); ?>" <?php if ($captcha_type == 'invisible') {
                    ...
                    wp_enqueue_script('g-recaptcha', 'https://www.recaptcha.net/recaptcha/api.js' . $args);
                    return;
    }
...
function dig_check_required($values)
{
    if (function_exists('is_checkout')) {
        if (is_checkout()) {
            if (
                ($values['meta_key'] == 'last_name' && !empty($_REQUEST['billing_first_name'])) ||
    ...
    foreach ($reg_custom_fields as $key => $values) {
        ...
        if (!$return) {
            ...
            if ($type == "textarea") {
                ...
                if ($type == "dropdown" && $user_role == 1) {
                    ...
                    ?>
                                                    <select name="digits_reg_<?php echo $meta_key; ?>" <?php echo $custom_class; ?>                                         <?php if ($values['required'] == 1) {
                                                        ...
                                                        if ($values['required'] == 1)
                                                            $dropdown_display_label = $dropdown_display_label . ' *';
                                                        ...
                                                        echo '<option value="-1" ' . $drop_required . ' ' . $selected . ' data-display="' . esc_attr($dropdown_display_label) . '">' . __('Select an option', 'digits') . '</option>';
                ...
                ?>
                                                    <div class="digits-input-wrapper">
                                                        <input type="<?php echo $inp_type; ?>" name="digits_reg_<?php echo $meta_key; ?>"
                                                            placeholder="<?php echo esc_attr($placeholder); ?>" id="digits_reg_<?php echo $meta_key . $rand; ?>" <?php echo $custom_class; ?>                                 <?php if ($values['required'] == 1) {
                                                                                                            echo "required";
...
function show_digcaptcha()
{
    ...
    ?>
            ...
            <img src="<?php echo $cap . '?r=' . $r; ?>" cap_src="<?php echo $cap; ?>" class="dig_captcha" draggable="false" />
            <?php
}
